...
def sanitize_filename(url: str) -> str:
    ...
    return (name[:100] or "screenshot") + ".png"
...
def main():
    ...
    try:
        r = requests.get(shot_url, timeout=30)
        if r.status_code != 200 or "image" not in r.headers.get("Content-Type","").lower():
            print("Screenshot service returned non-image or error. Status:", r.status_code)
            print("Body (first 200 chars):", r.text[:200])
            return
        out = sanitize_filename(url.replace("https://","").replace("http://",""))
